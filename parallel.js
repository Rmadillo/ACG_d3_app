...
var colors = {
  "Hospital A": [255,127,0],
  "Hospital B": [28,100,52],
  "Hospital C": [228,26,28],
  "Hospital D": [20,49,49],
  "Hospital E": [255,255,51]
};
...
d3.csv("ACG_data.csv", function(raw_data) {
  ...
  // Add a Medicare status element for each dimension.
...
function render_range(selection, i, max, opacity) {
  selection.slice(i,max).forEach(function(d) {
    path(d, foreground, color(d.Hospital,opacity));
  });
};
...
function data_table(sample) {
  ...
  table
    .append("span")
      ...
      .style("background", function(d) { return color(d.Hospital,0.85) })

  table
    .append("span")
      .text(function(d) { return d.id + ", RUB: " + d.RUB + ", " + d.Group + ", " + d.Hospital ; })
}
...
function highlight(d) {
  ...
  d3.selectAll(".row").style("opacity", function(p) { return (d.Hospital == p) ? null : "0.3" });
  path(d, highlighted, color(d.Hospital,1));
}
...
function brush() {
  ...
  var tallies = _(selected)
    .groupBy(function(d) { return d.Hospital; })
